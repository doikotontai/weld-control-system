...
export default function LoginPage() {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            const session = localStorage.getItem('weld-dev-session')
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        // DEV MODE: Login không cần Supabase
        if (DEV_MODE) {
            if (email.trim() === DEV_CREDENTIALS.email && password === DEV_CREDENTIALS.password) {
                localStorage.setItem('weld-dev-session', JSON.stringify({
                    id: 'dev-admin-001',
            ...
            <div style={{
                ...
                {DEV_MODE && (
                    <div style={{
                        ...
                        borderRadius: '8px', padding: '10px 14px', marginBottom: '20px', fontSize: '0.8rem',
                    }}>
                        <span style={{ color: '#fbbf24', fontWeight: 600 }}>⚠️ DEV MODE</span>
                        <br />
                        <span style={{ color: '#d97706', fontSize: '0.75rem' }}>Email: {DEV_CREDENTIALS.email}<br />Pass: {DEV_CREDENTIALS.password}</span>
                    </div>
                )}
